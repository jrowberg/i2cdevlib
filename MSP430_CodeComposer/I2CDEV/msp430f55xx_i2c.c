 ...
 *                  Changed incompatible register names from x261x to f55xx version
 *                  I2C_masterMultiByteReceiveStart() clears RX buffer before sending stop condition in f55xx.
 *                  (stop condition was sent first in x261x version)
 */
...
void I2C_masterSendSingleByte ( unsigned char txData   )
{
    ...
    unsigned char txieStatus = UCB1IE & UCTXIE;
    ...
    while (!(UCB1IFG & UCTXIFG)) ; //todo test removing this poll. unnecessary according to http://e2e.ti.com/support/microcontrollers/msp430/f/166/p/255954/897581.aspx
...
unsigned char I2C_masterMultiByteReceiveFinish ()
{
    ...
    //x261x:    Capture data from receive buffer after setting stop bit due to
    //          MSP430 I2C critical timing.
    //f55xx:    receive buffer before sending stop condition
    unsigned char result = UCB1RXBUF;
...
{
	if(UCB1IFG&UCTXIFG) {                       // Check for I2C TX
		//Check TX byte counter
		if (transmitCounter < TXLENGTH){ //TODO switch this to counting down to zero to avoid TXLENGTH variable
			//Initiate send of character from Master to Slave
			I2C_masterMultiByteSendNext(transmitData[transmitCounter]);
